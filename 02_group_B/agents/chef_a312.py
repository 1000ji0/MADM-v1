...
책임:
- 예산 제약을 준수하는 일식 레시피 생성 및 비용 최적화
...
class JapaneseChefBudgetAgent(BaseAgent):
    def __init__(self):
        ...
        super().__init__("A312", "Japanese Chef (Budget)", system_prompt)

    def create_recipe(self, requirement: Dict[str, Any]) -> Dict[str, Any]:
        query = requirement.get("menu_name", "저가 일식")
        rag = retrieve_recipe(query, "japanese")
        prices = get_market_prices(["면", "계란", "대파"])
        prompt = "예산 제약을 준수하는 일식 레시피를 JSON으로 작성하세요. 비용 내역과 절감 팁을 포함하세요."
