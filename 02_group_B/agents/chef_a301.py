...
책임:
- 건강 제약을 준수하는 한식 레시피 생성
- 제약 우선 필터링 및 템플릿 기반 조리법 제공
...
class KoreanChefHealthAgent(BaseAgent):
    def __init__(self):
        ...
        super().__init__("A301", "Korean Chef (Health)", system_prompt)

    def create_recipe(self, requirement: Dict[str, Any]) -> Dict[str, Any]:
        query = requirement.get("menu_name", "저염 한식")
        rag = retrieve_recipe(query, "korean")
        prompt = "요구사항을 만족하는 한식 레시피를 JSON으로 작성하세요. 건강 제약을 준수하고 대체재 근거를 포함하세요."
