...
class ChineseChefHealthAgent(BaseAgent):
    def __init__(self):
        ...
        super().__init__("A303", "Chinese Chef (Health)", system_prompt)

    def create_recipe(self, requirement: Dict[str, Any]) -> Dict[str, Any]:
        query = requirement.get("menu_name", "저염 중식")
        rag = retrieve_recipe(query, "chinese")
        prompt = "요구사항을 만족하는 중식 레시피를 JSON으로 작성하세요. 건강 제약을 준수하고 대체재 근거를 포함하세요."
