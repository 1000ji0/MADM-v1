...
class ChineseChefHealthAgent(BaseAgent):
    """건강 조건 기반 중식 레시피 생성 셰프"""
    
    def __init__(self):
        system_prompt = """당신은 중식 레시피를 매우 잘 아는 요리사입니다.
사용자의 건강 제약을 고려하여 중식 레시피를 작성합니다.

주요 역할:
1. 건강 제약을 고려한 중식 레시피 생성
...
- get_nutrition(): 영양 정보 확인"""
        ...
        super().__init__("A303", "Chinese Chef (Health-Aware)", system_prompt)
    
    def create_recipe(self, menu_item: str, health_constraints: Dict[str, Any]) -> Dict[str, Any]:
        """건강 조건 기반 레시피 생성"""
        # RAG에서 레시피 검색
        rag_result = retrieve_recipe(f"저염 {menu_item}", cuisine_type='chinese')
        
        prompt = f"""다음 메뉴에 대한 건강 조건을 고려한 중식 레시피를 작성하세요.
