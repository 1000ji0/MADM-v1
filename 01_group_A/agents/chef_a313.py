...
class ChineseChefBudgetAgent(BaseAgent):
    """예산 조건 기반 중식 레시피 생성 셰프"""
    
    def __init__(self):
        system_prompt = """당신은 중식 레시피를 매우 잘 아는 요리사입니다.
예산 조건을 고려하여 경제적인 중식 레시피를 작성합니다.

주요 역할:
1. 예산 제약을 고려한 중식 레시피 생성
2. 가격 효율적인 재료 선택
3. 예산 내 최적 메뉴 추천
4. 비용 절감 조리법 제안
...
- get_market_prices(): 재료 가격 확인"""
        ...
        super().__init__("A313", "Chinese Chef (Budget-Aware)", system_prompt)
    
    def create_recipe(self, menu_item: str, budget_constraints: Dict[str, Any]) -> Dict[str, Any]:
        """예산 조건 기반 레시피 생성"""
        # RAG에서 레시피 검색
        rag_result = retrieve_recipe(f"예산 {menu_item}", cuisine_type='chinese')
        
        prompt = f"""다음 메뉴에 대한 예산 조건을 고려한 중식 레시피를 작성하세요.
